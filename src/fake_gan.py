...
##############################################################################
    ...
    D = newmodels.Critic(1, 4, 128, use_bias = args.use_bias)
    ...
    for e in range(num_epochs):
        ...
        for batch_idx, sample in enumerate(train_dataloader):
            ...
            D_loss = - (torch.mean(D(target)) - torch.mean(D(fake_output)))
            ...
            #forward
            fake_output = A(net_input)
            A_loss = - torch.mean(D(fake_output))
            # loss = myloss(myoutput, target)
            # backward
            optimizerA.zero_grad()
            optimizerD.zero_grad()
            ...
            optimizerA.step()
            # update epoch loss
            A_train_loss += (D_loss.item() - D_train_loss) / num_train_samples
