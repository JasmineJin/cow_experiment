...
A = newmodels.MultiFilter(2, 64, 1, 4, last_act = torch.nn.Sigmoid, use_bias = False, use_dropout = False)
# down = A.down

# D = torch.nn.Sequential(
#     torch.nn.Linear(8 * 8 * 16, 256),
...
if __name__ == '__main__':
    writer_directory = os.path.join('runs', 'experiment_with_' + 'gans1')
    writer = SummaryWriter(writer_directory)
    
    net_input_name = 'log_partial'
    target_name = 'log_full'

    data_dir = os.path.join('cloud_data', 'vline', 'train')
    ...
    for it in range(num_iter):
        ...
        #   
            print('training D on batch', batch_idx)
            X = sample[net_input_name]
            X = mydata.norm01(X)
            ...
            D_loss = - torch.mean(torch.abs(D_real - D_fake))#-torch.mean(D_fake)
        ...
        D.eval()
        for batch_idx, sample in enumerate(mydataloader):
            print('training A on batch', batch_idx)
            X = sample[net_input_name]
            X = mydata.norm01(X)
            Y = sample[target_name]
            Y = mydata.norm01(Y)
            ...
            G_loss = torch.mean(torch.abs(D_real - D_fake))#-torch.mean(D_fake)
        ...
        if it % 1 == 0:
            writer.add_scalar('D_loss', D_loss, it)
            writer.add_scalar('G_loss', G_loss, it)
            print('Iter-{}; D_loss: {}; G_loss: {}'
                .format(it, D_loss.data.numpy(), G_loss.data.numpy()))

            # samples = G(z).data.numpy()[:16]
            Y_fake_np = Y_fake.detach().numpy()
            plt.figure()
            plt.imshow(Y_fake_np[0, 0, :, :], cmap = 'gray')
            plt.title('output')
            # plt.show()
            X_np = X.detach().numpy()
            plt.figure()
            ...
            plt.title('target')
            plt.show()
            torch.save(A,  'A' + str(it) + '.pt')
            torch.save(D,  'D' + str(it) + '.pt')
